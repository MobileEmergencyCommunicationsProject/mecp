...
namespace ManetMsgTypes
{
    ...
    enum MsgType
    {
        MSG_TYPE_NHDP                   = 1, //TBD nhdp document
        MSG_TYPE_DYMO                   = 2, //TBD dymo document
        MSG_TYPE_OLSRv2                 = 3  //TBD olsrv2 document
    };
    ...
    enum MsgTlv
    {
        ...
        MSG_TLV_NHDP_SMF               = 128  //TBD smf document
    };
    enum SubTypeSmfRelayAlgorithmID
    {
        SUB_TYPE_SMF_CF                = 0, //TBD smf document
        SUB_TYPE_SMF_SMPR              = 1, //TBD smf document
        SUB_TYPE_SMF_ECDS              = 2, //TBD smf document
        SUB_TYPE_SMF_MPRCDS            = 3,  //TBD smf document
        SUB_TYPE_SMF_ECDS_ETX          = 128 //TBD smf document
    };
    ...
    enum AddrBlkTlv
    {
        ...
        ADDR_TLV_LOCAL_IF              = 2, //TBD nhdp document
        ADDR_TLV_LINK_STATUS           = 3, //TBD nhdp document
        ADDR_TLV_OTHER_NEIGHB          = 4, //TBD nhdp document
        ADDR_TLV_NHDP_SMF              = 128, //TBD smf document
...
class ManetTlv : public ProtoPkt
{
    public:
        ...
        bool SetIndexRange(UINT8 start, UINT8 stop, bool multivalue = false);
        ...
        **/
        ...
        bool UpdateIndexRange(UINT8 start, UINT8 stop, bool multivalue);
        ...
        UINT8 GetIndexStart() const
            {return GetUINT8(OffsetIndexStart());}
        UINT8 GetIndexStop() const
            {return (HasSingleIndex() ? GetIndexStart() : GetUINT8(OffsetIndexStop()));}
        ...
        **/
        bool GetValue(char* value, UINT16 valueLength, int index = -1) const;

        /**
         * Here are some "shortcut" methods for certain lengths. 
         * Index is related to the address block field and not the value field.
        **/
        bool GetValue(UINT8& value, int index = -1) const
            {return GetValue((char*)&value, sizeof(UINT8), index);}
        bool GetValue(UINT16& value, int index = -1) const
        {
            bool result = GetValue((char*)&value, sizeof(UINT16), index); 
            value = ntohs(value);
            return result;
        }
        bool GetValue(UINT32& value, int index = -1) const
        {
            bool result = GetValue((char*)&value, sizeof(UINT32), index); 
            value = ntohl(value);
            return result;
        }

        // This returns a pointer to the value portion of the tlv buffer_ptr
        const char* GetValuePtr(UINT16 valueLength = 0, int index = -1) const;
