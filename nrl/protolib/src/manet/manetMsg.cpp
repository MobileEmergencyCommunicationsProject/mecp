...
bool ManetTlv::SetIndexRange(UINT8 start, UINT8 stop, bool multivalue)
{
    ASSERT(start <= stop);
    if (start == stop)
    {
...
bool ManetTlv::UpdateIndexRange(UINT8 start, UINT8 stop, bool multivalue)
{
    ...
    if (start == stop)
    {
        if (GetIndexStart() != GetIndexStop())
        {
            // we need to slide any content backwards one byte
            unsigned int contentStart = OffsetIndexStart() + 1;
            unsigned int contentLength = pkt_length - contentStart - 1;
            memmove((char*)buffer_ptr+contentStart, (char*)buffer_ptr+contentStart+1, contentLength);
            pkt_length--; 
        }
        ClearSemantics(MULTI_INDEX | MULTIVALUE);
        SetSemantic(SINGLE_INDEX);
    ...
    else
    {
        if (GetIndexStart() == GetIndexStop())
        {
            // we need to slide any content forwards one byte
        ...
        SetSemantic(MULTI_INDEX);
        if (multivalue) 
            SetSemantic(MULTIVALUE);
        else
            ClearSemantic(MULTIVALUE);
        SetUINT8(OffsetIndexStart(), start);
        SetUINT8(OffsetIndexStop(), stop);
    }
    return true;
...
// Note: User _must_ call SetIndexRange() _before_ this method for "index" >= 0
bool ManetTlv::SetValue(const char* value, UINT16 valueLength, int index)
{
    if (index < 0)
    {
    ...
    else if (!SemanticIsSet(SINGLE_INDEX) && !SemanticIsSet(MULTI_INDEX))
    {
        ASSERT(index < 0);
        if (index >= 0)
        {
    ...
    else
    {
        ...
        if ((index < startIndex) || (index > GetIndexStop()))
        {
...
UINT16 ManetTlv::GetValueLength() const
{
    UINT16 tlvLength = GetTlvLength();
    if (IsMultiValue() && (0 != tlvLength))
    {
        ASSERT(GetIndexStop() >= GetIndexStart());
        int numberValues = GetIndexStop() - GetIndexStart() + 1;
        if (numberValues > 0)
        {
...
bool ManetTlv::GetValue(char* value, UINT16 valueLength, int index) const
{
    const char* valuePtr = GetValuePtr(valueLength, index);
    if (NULL != valuePtr)
    {
...
const char* ManetTlv::GetValuePtr(UINT16 valueLength, int index) const
{
    if (!HasValue())
    ...
    ASSERT((0 == index) || (valueLength == GetValueLength()));
...
bool ManetAddrBlock::InitIntoBuffer(char* bufferPtr, unsigned int numBytes, bool freeOnDestruct)
{
    ...
    SetAddressCount(0);
//    SetSemantics((UINT8)(NO_HEAD | NO_TAIL));
...
bool ManetPkt::AppendMessage(ManetMsg& msg)
{
    ...
    if (buffer_bytes < newLength) return false;
    memcpy((char*)buffer_bytes + pkt_length, (char*)msg.GetBuffer(), msgLength);
    pkt_length = newLength;
    return true;
}  // end ManetPkt::AppendMessage()
...
bool ManetPkt::GetNextMessage(ManetMsg& msg)
{
    ...
    // Consume any padding bytes preceding the message
    while ((nextOffset < pkt_length) && (0 == *nextBuffer))
    {
        nextBuffer++;
        nextOffset++;
    }
