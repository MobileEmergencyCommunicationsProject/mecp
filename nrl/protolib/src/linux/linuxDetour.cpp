                              ...
                              unsigned int        dstFilterMask)
{
    ...
    {
#if defined(MAEMO)
	// Fremantle extras package puts iptables in /usr/sbin
	cmd = "/usr/sbin/iptables -t mangle";  // IPv4 iptables
#else
...
#if defined(MAEMO)
	// Fremantle extras package puts iptables in /usr/sbin
	cmd = "/usr/sbin/ip6tables -t mangle";
#else
                       ...
                       unsigned int        dstFilterMask)
{
    ...
    {
        PLOG(PL_ERROR, "LinuxDetour::Open() nfq_unbind_pf() error: %s\n", GetErrorString());   
        Close();
        return false;
    }
