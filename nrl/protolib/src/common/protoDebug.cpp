...
{
#ifdef MACOSX
    // It seems that "debug_log" is not always getting initialized on MACOSX
    // before constructors, etc are called (some sort of optimization?)
    if (NULL == debug_log) debug_log = stderr;
#endif // MACOSX    
    ...
    if ((level < old_level) && (debug_log != NULL))
...
bool OpenDebugLog(const char *path)
{
    ...
    if (NULL == debug_log) debug_log = stderr;
#endif // MACOSX    
    PLOG(PL_ERROR,"ProtoDebug>OpenDebugLog: debug log is being set to \"%s\"\n",path);
...
{
    if (((unsigned int)level <= debug_level) || (PL_ALWAYS == level)) 
    {
#ifdef MACOSX
    // It seems that "debug_log" is not always getting initialized on MACOSX
    // before constructors, etc are called (some sort of optimization?)
...
#ifdef _WIN32_WCE
        if (debug_window.IsOpen() && !debug_pipe.IsOpen() && ((stderr == debug_log) || (stdout == debug_log)))
        {
