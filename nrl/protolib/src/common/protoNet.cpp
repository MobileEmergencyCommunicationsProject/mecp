				                  ...
				                  ProtoAddressList&   addrList)
{
    ...
    if (NULL == ifIndices)
    {
        PLOG(PL_WARN, "ProtoNet::GetHostAddressList() new ifIndices[] error: %s\n", GetErrorString());
        return false;
    }
	...
	{   
		...
		if (GetInterfaceName(ifIndices[i], ifName, 255))
        {
            ...
            {
                PLOG(PL_WARN, "ProtoNet::GetHostAddressList() error: unable to get addresses for iface index %d\n", ifIndices[i]);
            }
        }
        else
        {
            PLOG(PL_WARN, "ProtoNet::GetHostAddressList() error: unable to get name for iface index %d\n", ifIndices[i]);
        } 
    }
    delete[] ifIndices;
