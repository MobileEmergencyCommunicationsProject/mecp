...
bool ProtoAddress::IsLoopback() const
{
    switch(type)
    {
        case IPv4:
        {
            ...
            UINT32 inAddr = ((struct sockaddr_in*)&addr)->sin_addr.s_addr;
            return (0x7f == (ntohl(inAddr) >> 24));
        }
#ifdef HAVE_IPV6
        case IPv6:
        {
...
#endif // HAVE_IPV6
        default:
            PLOG(PL_WARN, "ProtoAddress::SetSockAddr() Invalid address type!\n");
            type = INVALID;
            length = 0;
            return false;
    }
...
bool ProtoAddress::ResolveEthFromString(const char* text)
{
    ...
    {
        PLOG(PL_ERROR, "ProtoAddress: ResolveEthFromString(): Invalid ETH address type!\n");
        return false;
    }
    for (int i=0;i<=5;i++)
        b[i] = (char) a[i];
    ...
    if (hp)
    {
        ...
        if (alias && *alias && buffer)
        {
            ...
            while (*alias)
            {
                ...
                unsigned int alen = strlen(*alias);
    ...
    else
    {
        ...
        if (!lookupOK || IsLoopback())
        {
            ...
            if (!ProtoSocket::FindLocalAddress(IPv4, *this))
            {
                ...
                {
                    PLOG(PL_ERROR, "ProtoAddress::ResolveLocalAddress() warning: no assigned addresses found\n");
                }
#endif // HAVE_IPV6
            }
