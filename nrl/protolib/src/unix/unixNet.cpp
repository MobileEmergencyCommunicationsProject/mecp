                                       ...
                                       unsigned int*       interfaceIndex)
{
    ...
    if (ioctl(socketFd, SIOCGIFADDR, &req) < 0)
    {
        ...
        PLOG(PL_WARN, "ProtoNet::GetInterfaceAddressList() ioctl(SIOCGIFADDR) error for iface>%s: %s\n",
    ...
    {
        PLOG(PL_ERROR, "ProtoNet::GetInterfaceAddressList() warning: no addresses for given family?\n");
        return false;
    }
    else 
#endif // MACOSX
