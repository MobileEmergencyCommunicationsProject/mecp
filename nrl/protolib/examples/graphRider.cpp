...
void Usage()
{
    fprintf(stderr, "Usage: gr {ns|sdt} <mobilityTraceFile> range <commsRange> [desgree]\n");
}
                                  ...
                                  ProtoGraph::Vertice::SortedList::ItemPool*    sortedVerticeItemPool)
{
    ...
    while (NULL != (iface = it.GetNextInterface()))
    {
        ...
        {
            bool xIsInN1 = x->HasEdgeTo(*iface); // neighbor of "n0"?
            ...
            while (NULL != (n = iteratorX.GetNextAdjacency()))
            {
                ...
                bool nIsInN1 = n->HasEdgeTo(*iface); // neighbor of "n0"?
