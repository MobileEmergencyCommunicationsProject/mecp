    ...
    while (iterator.GetNextMessage(recvMsg))
    {
        ...
        while (addrBlkIterator.GetNextAddressBlock(recvAddrBlock))
        {
            TRACE("    got addr block len:%u w/ %u addresses (len:%u)\n",
                  recvAddrBlock.GetLength(), recvAddrBlock.GetAddressCount());
            ...
            while (iterator.GetNextTlv(recvTlv))
            {
                ...
                if (recvTlv.HasIndex())
                {
                    ...
                    stop = recvTlv.GetIndexStop();

                }
                else
                {
                    start = 0;
                    stop = recvAddrBlock.GetAddressCount() - 1;
                }
                ...
                {
                    TRACE("      found %ssingle-value addr-blk-tlv, type:%d len:%u indices:%u-%u\n",
                          recvTlv.HasIndex() ? "indexed, " : "non-", recvTlv.GetType(), recvTlv.GetLength(),
                          start, stop);
                    start = stop;  // to force print out of _single_ value
                }
                for (UINT8 index = start; index <= stop; index++)
                {
                    switch (recvTlv.GetValueLength())
                    {
                        case 1:
                        {
                            UINT8 val;
                            recvTlv.GetValue(val, index);
                        ...
                        default:
                        {
                            UINT8* vptr = (UINT8*)recvTlv.GetValuePtr();
                            TRACE("        value[%u] = 0x\n", index);
                            for (unsigned int i = 0; i < recvTlv.GetValueLength(); i++)
                                TRACE("%02x", *vptr++);
                            TRACE("\n");
                            break;
                        }
