	                      ...
	                      ProtoChannel::Notification notifyType)
{
    ...
    while(1) 
    {
        ...
        const unsigned int BUFFER_MAX = 4096 + (256 *sizeof(UINT32));
       ...
       // Map ProtoPktETH instance into buffer and init for processing
        ...
        {
            PLOG(PL_ERROR, "SmfApp::OnPktCapture() error: bad Ether frame\n");
            continue;
        }
        // Only process IP packets (skip others)
        ProtoPktETH::Type ethType = (ProtoPktETH::Type)ethPkt.GetType();
        if ((ethType != ProtoPktETH::IP) && (ethType != ProtoPktETH::IPv6)) continue;
    ...
    // In "MNE" environment, ignore packets from blocked MAC sources
        ...
        unsigned int srcIfIndex = (int)cap.GetInterfaceIndex();
        unsigned int dstIfIndices[IF_COUNT_MAX];
        ...
        if (dstCount > 0)
        {
            ...
            if (tap_active)
            {
                ...
                {
                    PLOG(PL_ERROR, "SmfApp::OnPktCapture() error: unable to send packet to \"tap\" process\n");
                }
            } 
#ifdef _PROTO_DETOUR
            else if (firewall_forward)
            {
                ...
                {
                    PLOG(PL_ERROR, "SmfApp::OnPktCapture() error: unable to forward packet via ProtoDetour\n");
                } 
            }
#endif // _PROTO_DETOUR
            else
            {
                ...
                {
                    PLOG(PL_ERROR, "SmfApp::OnPktCapture() error: unable to forward packet via ProtoCap device\n");
                }
            }
        }  // end if (dstCount > 0)
...
bool SmfApp::ForwardFrame(unsigned int dstCount, unsigned int* dstIfIndices, char* frameBuffer, unsigned int frameLength)
{
    ...
    for (unsigned int i = 0; i < dstCount; i++)
    {
        ...
        if (!dstCap->Forward(frameBuffer, frameLength))
        {
            PLOG(PL_ERROR, "SmfApp::ForwardFrame() error: unable to send frame via iface index: %d\n", dstIfIndex);
...
// (NOTE: This is use ONLY when the "firewallForward" option is used!!!)
            ...
            if (0 != numBytes)
            {
                ...
                switch (direction)
                {
                    ...
                    case ProtoDetour::INBOUND:
                    {
                        ...
                        // Finally, process packet for possible forwarding given ipPkt, srcMacAddr, and srcIfIndex    
                        unsigned int dstIfIndices[IF_COUNT_MAX];
                        unsigned int dstCount = smf.ProcessPacket(ipPkt, srcMacAddr, ifIndex, dstIfIndices, IF_COUNT_MAX);
                        if (dstCount > 0)
                        {
                            ...
                            {
                                if (!ForwardFrame(dstCount, dstIfIndices, (char*)ethBuffer, ProtoPktETH::HDR_LEN + ipPkt.GetLength()))
                                    PLOG(PL_ERROR, "SmfApp::OnPktIntercept() error: unable to forward packet via ProtoCap device\n");
                            }
                        }  // end if (dstCount > 0)
                        break;
