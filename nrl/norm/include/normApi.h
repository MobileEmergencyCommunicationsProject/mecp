...
NORM_API_LINKAGE 
bool NormGetNextEvent(NormInstanceHandle instanceHandle, NormEvent* theEvent, bool waitForEvent = true);
...
bool NormSetTxPort(NormSessionHandle sessionHandle,
                   ...
                   const char*       txBindAddress = (const char*)0);  // if non-NULL, bind() to <txBindAddress>/<txPortNumber>
...
void NormSetRxPortReuse(NormSessionHandle sessionHandle,
                        bool              enableReuse,
                        const char*       rxBindAddress = (const char*)0, // if non-NULL, bind() to <rxBindAddress>/<sessionPort>
                        const char*       senderAddress = (const char*)0, // if non-NULL, connect() to <senderAddress>/<senderPort>
                        UINT16            senderPort = 0);
...
void NormSetEcnSupport(NormSessionHandle  sessionHandle, 
                       ...
                       bool               ignoreLoss = false,    // With "ecnEnable", use ECN-only, ignoring packet loss
                       bool               tolerateLoss = false); // loss-tolerant congestion control, ecnEnable or not, ignoreLoss = false
