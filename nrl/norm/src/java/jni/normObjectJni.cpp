...
JNIEXPORT jobject JNICALL PKGNAME(NormObject_getType)
  ...
  jobjectArray array = (jobjectArray)env->CallStaticObjectMethod(
    cid_NormObjectType, mid_NormObjectType_values);
...
JNIEXPORT jobject JNICALL PKGNAME(NormObject_getSender)
  ...
  if (nodeHandle == NORM_NODE_INVALID) {
    env->ThrowNew(cid_IOException, "Locally originated sender object");
    return NULL;
  }

  // Create the NormNode
  return env->NewObject(cid_NormNode, mid_NormNode_init,
    (jlong)nodeHandle);
}
