...
NormApp::NormApp()
 : control_pipe(ProtoPipe::MESSAGE), control_remote(false), 
   ...
   tx_rate(64000.0), tx_rate_min(-1.0), tx_rate_max(-1.0), cc_enable(false), ecn_mode(ECN_OFF),
...
bool NormApp::OnCommand(const char* cmd, const char* val)
{
    ...
    else if (!strncmp("ecn", cmd, len))
    {
        ...
        {
            ecn_mode = ECN_ON;
            if (session) session->SetEcnSupport(true, false);
        }
        else if (!strcmp("off", val))
        {
            ecn_mode = ECN_OFF;   
            if (session) session->SetEcnSupport(false, false);
        }
        else if (!strcmp("only", val))
        {
            ecn_mode = ECN_ONLY;   
            if (session) session->SetEcnSupport(true, true);
        }
        else
        {
...
bool NormApp::OnStartup(int argc, const char*const* argv)
{
    ...
    if (session)
    {
        ...
        {
            case ECN_OFF:
                session->SetEcnSupport(false, false);
                break;
            case ECN_ON:
                session->SetEcnSupport(true, false);
                break;
            case ECN_ONLY:
                session->SetEcnSupport(true, true);
                break;
        }
