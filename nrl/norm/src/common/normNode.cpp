...
void NormSenderNode::HandleObjectMessage(const NormObjectMsg& msg)
{
    ...
    else
    {
        ...
        UINT8 fecId = data.GetFecId();
        if (BuffersAllocated())
        {
            if (fec_id == fecId)
                allocateBuffers = false;  // TBD - validate fec params?
            else
                FreeBuffers(); // force reallocation
        }
            ...
            if (GetFirstPending(nextId))
            {
                ...
                if (repairPending)
                {
                    ...
                    if (cc_enable)
                    {
                        ...
                        PLOG(PL_TRACE, "NormSenderNode::OnRepairTimeout() node>%lu sending NACK rate:%lf kbps (rtt:%lf loss:%lf s:%hu) slow_start:%d\n",
...
void NormSenderNode::AttachCCFeedback(NormAckMsg& ack)
{
    ...
    //DMSG(0, "NormSenderNode::AttachCCFeedback() node>%lu sending ACK rate:%lf kbps (rtt:%lf loss:%lf s:%lf recvRate:%lf) slow_start:%d\n",
    //               LocalNodeId(), 8.0e-03*NormUnquantizeRate(ext.GetCCRate()) , 
                                ...
                                bool                    ecnStatus)
{
    ...
    // of the last loss event and the current sequence number?
