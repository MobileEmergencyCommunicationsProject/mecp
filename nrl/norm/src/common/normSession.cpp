...
NormSession::NormSession(NormSessionMgr& sessionMgr, NormNodeId localNodeId) 
 : session_mgr(sessionMgr), notify_pending(false), tx_port(0), tx_port_reuse(false),
   ...
   ttl(DEFAULT_TTL), tos(0), loopback(false), ecn_enabled(false), 
   ...
   acking_node_count(0), watermark_pending(false), watermark_flushes(false),
   ...
   cmd_count(0), cmd_buffer(NULL), cmd_length(0), 
...
bool NormSession::Open()
{
    ...
    if (!report_timer.IsActive()) ActivateTimer(report_timer);
...
void NormSession::Close()
{
    ...
    if (is_sender) StopSender();
    if (is_receiver) StopReceiver();
...
bool NormSession::SetTxPort(UINT16 txPort, bool enableReuse, const char* txAddress) 
{
    ...
    if (NULL != txAddress)
    {
        ...
        if (result)
        {
            if ((tx_port == address.GetPort()) && !tx_address.HostIsEqual(address))
...
bool NormSession::ChangeSessionAddress(const char* sessionAddr, UINT16 sessionPort)
{
    if (tx_only || !IsOpen())
    {
        ProtoAddress theAddr;
        if (theAddr.ResolveFromString(sessionAddr))
        {
            address = theAddr;
            address.SetPort(sessionPort);
            return true;
        }
        else
        {
            PLOG(PL_ERROR, "NormSession::ChangeSessionAddress() error: unable to resolve address \"%s\"\n", sessionAddr);   
        }
    }
    else
    {
        PLOG(PL_ERROR, "NormSession::ChangeSessionAddress() error: session is open and not tx-only!\n");
    }
    return false;
}  // end NormSession::SetSessionAddress()
...
bool NormSession::SenderAddAckingNode(NormNodeId nodeId)
{
    ...
    if (NULL == theNode)
    {
        ...
        {
            PLOG(PL_ERROR, "NormSession::AddAckingNode() new NormAckingNode error: %s\n", GetErrorString());
        }                
    }
    else
    {
        PLOG(PL_ERROR, "NormSession::AddAckingNode() warning: node already in list!?\n");
    }
    return true;
                                         ...
                                         UINT16      infoLen)
{
    ...
    NormDataObject* obj = new NormDataObject(*this, (NormSenderNode*)NULL, next_tx_object_id);
    if (!obj)
    {
               ...
               UINT8                    fecM)
{
    ...
    PLOG(PL_ALWAYS, "node>%lu %s>%s ", (UINT32)localId, status, addr.GetHostString());
...
{   
    ...
    switch (msg.GetType())
    {
        case NormMsg::INFO:
            ...
            if (IsReceiver()) ReceiverHandleObjectMessage(currentTime, (NormObjectMsg&)msg, ecnStatus);
            break;
        case NormMsg::DATA:
        ...
        case NormMsg::CMD:
            ...
            if (IsReceiver()) ReceiverHandleCommand(currentTime, (NormCmdMsg&)msg, ecnStatus);
            break;
        case NormMsg::NACK:
                                         ...
                                         UINT16          ccSequence)       
{
     ...
     PLOG(PL_DEBUG, "NormSession::SenderHandleCCFeedback() cc feedback recvd at time %lu.%lf  ccRate:%9.3lf ccRtt:%lf ccLoss:%lf\n", 
             (unsigned long)currentTime.tv_sec, ((double)currentTime.tv_usec)*1.0e-06,
             ccRate*8.0/1000.0, ccRtt, ccLoss);
    ...
    PLOG(PL_DEBUG, "NormSession::SenderHandleCCFeedback() node>%lu rate>%lf (rtt>%lf loss>%lf slow_start>%d limit>%d)\n",
            nodeId, ccRate * 8.0 / 1000.0, ccRtt, ccLoss, (0 != (ccFlags & NormCC::START)), (0 != (ccFlags & NormCC::LIMIT)));
...
void NormSession::SenderHandleAckMessage(const struct timeval& currentTime, const NormAckMsg& ack, bool wasUnicast)
{
    ...
    if (receiverRtt >= 0.0) SenderUpdateGrttEstimate(receiverRtt);
    ...
    while (ack.GetNextExtension(ext))
    {
        if (NormHeaderExtension::CC_FEEDBACK == ext.GetType())
        {
            SenderHandleCCFeedback(currentTime,
                                   ...
                                   ext.GetCCFlags(),
                                   receiverRtt >= 0.0 ?  
                                        receiverRtt : NormUnquantizeRtt(ext.GetCCRtt()),
    ...
    switch (ack.GetAckType())
    {
        ...
        default: 
            PLOG(PL_ERROR, "NormSession::SenderHandleAckMessage() node>%lu received "
...
void NormSession::SenderCancelCmd()
{
    if (0 != cmd_count)
    {
        if (cmd_timer.IsActive()) cmd_timer.Deactivate();
        cmd_count = 0;
        cmd_length = 0;
    }
...
bool NormSession::OnFlowControlTimeout(ProtoTimer& theTimer)
{
    ...
    // ProtoTime precision limitations put us into an infinite loop in
...
bool NormSession::OnTxTimeout(ProtoTimer& /*theTimer*/)
{
    ...
    if (msg)
    {
        ...
        else
        {
            ...
            // old code - time out and try again
            /*
            if (tx_rate > 0.0)
                //tx_timer.SetInterval(msg->GetLength() / tx_rate);
                tx_timer.SetInterval(GetTxInterval(msg->GetLength(), tx_rate));
            else if (0.0 == tx_timer.GetInterval())
                tx_timer.SetInterval(0.01);
            return true;
            */     
            // new code - use async i/o to cue retry
...
{   
    ...
    if (isReceiverMsg && receiver_silent)
    {
        ...
        return true; // wasn't sent but wasn't supposed to
    }
    else if (drop)
    {
...
double NormSession::GetProbeInterval()
{
    if (cc_enable && data_active)
    {
        ...
        if (NULL != clr)
        {
            ...
            if (clr->GetRtt() > 0.200)
            {
                if (probeCount > 3) probeCount = 3;
            }
            else if (clr->GetRtt() > 0.100)
            {
                if (probeCount > 2) probeCount = 2;
            }
            else
            {
                probeCount = 1;
        ...
        if (data_active)  // adjust only if actively transmitting
        {
            ...
            if (cc_slow_start)
            {
                ...
                if (GetDebugLevel() >= 6)
                {
                    ...
                    //TRACE("NormSession::AdjustRate(slow start) clr>%lu newRate>%lf (oldRate>%lf sentRate>%lf clrRate>%lf\n",
                    //        clr->GetId(), 8.0e-03*txRate,  8.0e-03*tx_rate, sentRate, 8.0e-03*clr->GetRate());
                }                          
            }
            else
            {
                ...
                //TRACE("NormSession::AdjustRate(stdy state) clr>%lu newRate>%lf (rtt>%lf loss>%lf clrRate>%9.3lf damper>%lf)\n",
                //     clr->GetId(), 8.0e-03*txRate, clr->GetRtt(), clr->GetLoss(), 8.0e-03*clr->GetRate(), damper);
            }
        }
        if (!address.IsMulticast())
        {
