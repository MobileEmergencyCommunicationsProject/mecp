...
NormSimAgent::NormSimAgent(ProtoTimerMgr&         timerMgr,
  ...
  : msg_sink(NULL), session_mgr(timerMgr, socketNotifier), session(NULL),
    ...
    cc_enable(false), ecn_enable(false), unicast_nacks(false), silent_receiver(false),
...
bool NormSimAgent::ProcessCommand(const char* cmd, const char* val)
{
    ...
    else if (!strncmp("ecn", cmd, len))
    {
        ...
        // Note we enable "ignoreLoss" here when ECN is enabled
        if (session) session->SetEcnSupport(ecn_enable, true, false);
        return true;
    }
...
bool NormSimAgent::StartSender()
{
    ...
    if (session)
    {
        ...
        session->SetEcnSupport(ecn_enable, true, false);
