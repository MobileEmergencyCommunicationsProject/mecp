...
bool NormObject::NextSenderMsg(NormObjectMsg* msg)
{
    ...
    if (segmentId < numData)
    {
        ...
        if (0 == payloadLength)
        {
            ...
            {
                PLOG(PL_WARN, "NormObject::NextSenderMsg() node>%lu Warning! can't repair old stream segment\n", LocalNodeId());
                block->UnsetPending(segmentId); 
                if (!block->IsPending())
                {
                                     ...
                                     char*            buffer)
{
    ...
    if (!block)
    {
        ...
        if (!stream_buffer.IsEmpty() && (blockId < stream_buffer.RangeLo()))
            PLOG(PL_FATAL, "NormStreamObject::ReadSegment() error: attempted to read old block> %lu\n", (UINT32)blockId);
