...
int main(int argc, char* argv[])
{
    ...
    NormSetTxRate(session, 8e+06);  // in bits/second
    ...
    //    (we enqueue text strings of random length)
    unsigned int dataCount = 0;
    char data = 'a';
    const int MIN_LENGTH = 100000;
    const int MAX_LENGTH = 500000;
    int dataLen = MIN_LENGTH + rand() % (MAX_LENGTH - MIN_LENGTH) + 1;
    char* dataMsg = new char[dataLen];
    ASSERT(NULL != dataMsg);
    memset(dataMsg, data, dataLen);  // set message content
